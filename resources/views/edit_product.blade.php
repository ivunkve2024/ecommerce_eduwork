...
<body class="bg-light">
            ...
            <div class="col-md-6">
                <div class="card shadow-sm border-0 rounded-3 p-4 bg-white">
                    ...
                    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-4">
                            <label class="form-label small fw-bold d-block">Gambar Produk</label>
                            
                            <div class="mb-3">
                                <img id="imagePreview" src="{{ $product->image ? asset('images/' . $product->image) : asset('images/default.jpg') }}" alt="{{ $product->name }}" class="img-thumbnail" style="width: 120px; height: 90px; object-fit: cover;">
                                <div class="form-text small text-muted mt-1" id="previewLabel">Gambar yang digunakan saat ini.</div>
                            </div>

                            <input type="file" id="imageInput" class="form-control" accept=".jpg, .jpeg, .png, .webp">
                            
                            <input type="hidden" name="image" id="croppedImageBase64">
                            <div class="form-text small text-muted">Kosongkan jika tidak ingin mengubah gambar. Format: JPG, JPEG, PNG (Maks 2MB).</div>
                        </div>
    ...
    <script>
        ...
        // Simpan URL gambar bawaan database sebagai cadangan jika batal mengganti gambar
        ...
        document.getElementById('btnCropAndSave').addEventListener('click', function () {
            if (cropper) {
                ...
                // Tampilkan hasil potongan baru ke elemen gambar preview
                imagePreview.src = base64Data;
                previewLabel.innerHTML = '<span class="text-success fw-bold">✓ Gambar baru siap diunggah!</span>';

                cropperModal.hide();
            }
        });

        // Fungsi Tombol Batal: Kembalikan kondisi pratinjau seperti semula
        const batalkan = () => { 
            imageInput.value = ""; 
            croppedImageBase64.value = "";
            imagePreview.src = originalImageSrc;
            previewLabel.innerText = "Gambar yang digunakan saat ini.";
            cropperModal.hide(); 
        };
